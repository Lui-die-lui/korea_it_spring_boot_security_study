package com.koreait.spirngSecurityStudy.security.jwt;
...
public class JwtUtil {
    ...
    private final Key KEY; // final 상수니까 대문자

    public JwtUtil(@Value("${jwt.secret}") String secret) { // secret 안에 암호 키 넣음
    ...
    public boolean isBearer(String token) {
        ...
        if (!token.startsWith("Bearer ")) { // Bearer 로 시작안하면
            return false; // false
        }
        return true; // 시작하면 true
    }

    public String removeBearer(String bearerToken) {
        return bearerToken.replaceFirst("Bearer ",""); // 가장 첫번째 "Bearer " 를 ""(빈 값)로 바꿔줌
    }
